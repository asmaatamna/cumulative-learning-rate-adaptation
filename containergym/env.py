...
class ContainerEnv(gym.Env):
    ...
    def __init__(
            ...
            # TODO: New param. determining how press timers are encoded (True: seconds, False: number of timesteps (
            #  integer))
            press_timer_in_sec: bool = False
    ):
        # Create environment's random number generator
        ...
        self.press_timer_in_sec = press_timer_in_sec  # TODO: Initialize press timer type
        ...
        self.reward_evaluator = RewardEvaluator(
            container_params=reward_params, r_pen=r_pen
        )
        ...
        self.color_code = self.get_color_code()
    ...
    def from_json(cls, filepath):
        ...
        with open(filepath, "r") as f:
            ...
            obj = cls(
                ...
                r_pen=data.get("R_PEN", -1e-1),
                press_timer_in_sec=data.get("PRESS_TIMER_IN_SEC", False)
                # TODO: Add "PRESS_TIMER_IN_SEC" to JSON config files when False
            )
            return obj
    ...
    def step(self, action):
        ...
        if action > 0:
            ...
            if free_presses:
                press_idx = self.rng.choice(free_presses)  # Uniform random choice
                ...
                if t_pressing_ends is not None:
                    ...
                    if self.press_timer_in_sec:
                        self.state.press_times[press_idx] = t_pressing_ends
                    else:
                        self.state.press_times[press_idx] = -(
                                    t_pressing_ends // -self.timestep)  # TODO: was t_pressing_ends
            else:
        ...
        if self.press_timer_in_sec:
            ...
            self.state.press_times = np.clip(
                self.state.press_times - 1, a_min=0, a_max=None
                # TODO: was self.state.press_times - self.timestep, a_min=0, a_max=None
            )
        ...
        if not (self.state.volumes < self.max_volumes).all():
            ...
            terminated = True
            current_reward = self.r_min  # apply minimum reward

        if self.state.episode_length == self.max_episode_length:
            # Max episode length is reached
            truncated = True
            # TODO: consider returning a large positive reward if episode ends without overflow
    ...
    def reset(self, seed=None, options=None):
        ...
        self.random_walk.rng = np.random.default_rng(seed=seed)

        self.state = State(self.n_containers, self.n_presses)
        self.state.reset(self.min_starting_volume, self.max_starting_volume, seed=seed)

        return self.state.to_dict(), {}

    def render(self):
        """
        Live plotting of volumes.
        """
        pass
...
class State:
    ...
    def __init__(self, enabled_containers, n_presses):
        ...
        n_presses : int
            Number of presses
        """
        # Create state's random number generator
        self.rng = np.random.default_rng()
    ...
    def reset(self, min_volumes, max_volumes, seed=None):
        """
