...
class ContainerEnv(gym.Env):
    ...
    def __init__(
        ...
        # Needed by render() function
        self.df_volumes = pd.DataFrame(columns=self.enabled_containers)
        self.df_volumes.loc[len(self.df_volumes)] = self.state.volumes
    ...
    def render(self):
        ...
        """
        plt.cla()
        self.df_volumes.plot(ax=self.ax,
                             color=[self.color_code[x] for x in self.df_volumes.columns],
                             xlim=(0, self.state.episode_length),  # self.max_episode_length
                             ylim=(0, max(self.max_volumes)),
                             xlabel="Timestep",
                             ylabel="Volume",
                             grid=True,
