...
class RewardEvaluator:
    ...
    def __init__(self, container_params: Union[dict, list], r_pen: float):
    ...
    def from_json(cls, filename):
        ...
        with open(filename) as json_file:
            ...
            return cls(container_params, r_pen)
    ...
    def gaussian_reward_dict_params(
        ...
        -------
        float
            A value between r_pen and 1, representing the reward for the given action.
        """
        reward = 0
        ...
        if action > 0:
            ...
            for i in range(len(params["peaks"])):
                ...
                reward += (height - self.r_pen) * np.exp(
                    -(current_volume - peak)
                    * (current_volume - peak)
                    / (2.0 * width * width)
                )
    ...
    def gaussian_reward_list_params(
        ...
        if action > 0:
            ...
            for i in range(len(container_params)):
                ...
                reward += (height - self.r_pend) * np.exp(
                    -(current_volume - peak)
                    * (current_volume - peak)
                    / (2.0 * width * width)
                )
